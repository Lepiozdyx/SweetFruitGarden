...
struct NewPlantErrorScreen: View {
    var body: some View {
    ...
    private var plantNameFieldError: some View {
        VStack(alignment: .leading, spacing: 6) {
            requiredLabel("Plant Name")
            formInput(iconAsset: nil, placeholder: "e.g. Apple tree by the fence", isError: true, focusedError: false)
            Text("Planting date is required")
                .font(.system(size: 12))
    ...
    private var plantingDateFieldError: some View {
        VStack(alignment: .leading, spacing: 6) {
            requiredLabel("Planting Date")
            formInput(iconAsset: "Icon-61", placeholder: "Select planting date…", isError: true, focusedError: true)
            Text("Planting date is required")
                .font(.system(size: 12))
                .foregroundStyle(Color.hex("FF1744"))
        }
    ...
    private func formInput(iconAsset: String?, placeholder: String, isError: Bool, focusedError: Bool) -> some View {
        HStack(spacing: 10) {
            if let iconAsset {
                Image(iconAsset)
                    .resizable()
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .stroke(isError ? Color.hex("FF1744") : Color.white.opacity(0.15), lineWidth: 1.5)
        )
        .overlay(
            Group {
                if focusedError {
                    RoundedRectangle(cornerRadius: 14)
                        .stroke(Color.hex("FF1744").opacity(0.15), lineWidth: 6)
                }
            }
        )
        .clipShape(RoundedRectangle(cornerRadius: 14))
